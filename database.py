...
class OutageDatabase:
    ...
    def _get_or_create_device(self, conn: sqlite3.Connection, device_key: str) -> int:
        ...
        if result:
            ...
            cursor = conn.execute(
                "INSERT INTO devices (device_key) VALUES (?)",
                (device_key,)
            )
    ...
    def _save_schedule(
        ...
        cursor = conn.execute("""
            ...
            WHERE device_id = ? AND schedule_date = ? AND is_today = ?
        """, (device_id, schedule_date, is_today))
        ...
        if existing:
            ...
            conn.execute("""
                ...
                SET status = ?, timezone_offset = ?, updated_on = ?
                WHERE id = ?
            """, (
